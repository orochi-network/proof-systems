...
impl<'a, 'b, C: AffineRepr> Add<&'a PolyComm<C>> for &'b PolyComm<C> {
    ...
    fn add(self, other: &'a PolyComm<C>) -> PolyComm<C> {
        ...
        for i in 0..std::cmp::max(n1, n2) {
            let pt = if i < n1 && i < n2 {
                self.unshifted[i].into_group() + other.unshifted[i].into_group()
            } else if i < n1 {
                self.unshifted[i].into_group()
            } else {
                other.unshifted[i].into_group()
            };
            unshifted.push(pt.into_affine());
        }
        let shifted = match (self.shifted, other.shifted) {
            (None, _) => other.shifted,
            (_, None) => self.shifted,
            (Some(p1), Some(p2)) => Some((p1 + p2).into_affine()),
        };
        PolyComm { unshifted, shifted }
    }
}
...
impl<'a, 'b, C: AffineRepr> Sub<&'a PolyComm<C>> for &'b PolyComm<C> {
    ...
    fn sub(self, other: &'a PolyComm<C>) -> PolyComm<C> {
        ...
        let shifted = match (self.shifted, other.shifted) {
            (None, _) => other.shifted,
            (_, None) => self.shifted,
            (Some(p1), Some(p2)) => Some((p1.into_group() + (-p2.into_group())).into_affine()),
        };
        PolyComm { unshifted, shifted }
    }
}
    ...
    <C as AffineRepr>::Group: VariableBaseMSM,
{
    ...
    pub fn multi_scalar_mul(com: &[&PolyComm<C>], elm: &[C::ScalarField]) -> Self {
        ...
        for chunk in 0..unshifted_size {
            ...
            let chunk_msm = C::Group::msm(&points, &scalars).expect("Unable to compute MSM");
    ...
    P::BaseField: PrimeField,
{
    ...
    fn of_coordinates(x: P::BaseField, y: P::BaseField) -> Affine<P> {
        Affine::<P>::new(x, y)
    }
...
impl<G: CommitmentCurve> SRS<G> {
    ...
    pub fn commit_non_hiding(
        ...
        let coeffs: Vec<_> = plnm.iter().map(|c| *c).collect();
        ...
        if is_zero {
            ...
            coeffs.chunks(self.g.len()).for_each(|coeffs_chunk| {
                let chunk = G::Group::msm(&self.g, coeffs_chunk)
                    .expect("Unable to perform MSM")
                    .into_affine();
        ...
        let shifted = match max {
            ...
            Some(max) => {
                ...
                if is_zero || start >= coeffs_len {
                    ...
                    let shifted =
                        G::Group::msm(&self.g[basis_len - (max % basis_len)..], &coeffs[start..]);
                    Some(shifted.expect("Unable to perform MSM").into_affine())
                }
            }
        };
